...
def strategyTEMP(Robots, kick, recebe):
    ...
    if BallHandler > 0:
        ...
        for i in range(len(solution)-2):
            guiElements.drawLine(Robots[solution[i]-1].position, Robots[solution[i+1]-1].position, color=COLORS["pink"], width=int(10*solutionProbs[i]))
        guiElements.drawLine(Robots[solution[-2]-1].position, (11, 0), color=COLORS["pink"], width=int(10*solutionProbs[-1]))
    ...
    distances = np.zeros((len(robotsLocationTemp), len(ZonesLocationTemp)))
    for i in range(len(robotsLocationTemp)):
        for j in range(len(ZonesLocationTemp)):
            distances[i, j] = np.linalg.norm(robotsLocationTemp[i] - ZonesLocationTemp[j])
    row_ind, col_ind = linear_sum_assignment(distances)
    min_distance = distances[row_ind, col_ind].sum()
    for i in range(len(row_ind)):
        # guiElements.drawLine(Robots[row_ind[i]].position, consts.ZonesPointsLoc[col_ind[i]], color=COLORS["blue"], width=3)
        Robots[row_ind[i]].probField[3].field = consts.zonesFields[col_ind[i]]
    ...
    # print(ballPath.x_pred[-1], ballPath.y_pred[-1], Robots[0].ball_position, ballPath.nextX, ballPath.nextY)
    nextBallPosition = [ballPath.nextXUI, ballPath.nextYUI]
    # ballPath.angle = np.degrees(np.arctan2(ballPath.nextYUI-ballPath.Y[-1], ballPath.nextXUI-ballPath.X[-1]))
    ...
    #BallHandler = 0
    if BallHandler == 0:
        if abs(round(ballPath.angle[-1]-robotAng)) < 10:
