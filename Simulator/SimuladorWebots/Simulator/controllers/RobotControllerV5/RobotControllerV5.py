...
robotNode = supervisor.getFromDef(RobotName)

if ROBOTID == 1:
    robotNode.getField('translation').setSFVec3f([-10, 0, 0.07])
    x = random.SystemRandom().uniform(-10, 10)
    y = random.SystemRandom().uniform(-6, 6)
    robot.ballNode.getField('translation').setSFVec3f([x, y, 0])
else:
    x = random.SystemRandom().uniform(-10, 10)
    y = random.SystemRandom().uniform(-6, 6)
    robotNode.getField('translation').setSFVec3f([x, y, 0.07])
...
#    robot.send2Display("drawOval", [0x000FF0, 1, 0.25], [coord2Dcoord([robot.position[0], robot.position[1]]), DIST2SEE*10, DIST2SEE*10])
    ...
    robot.dribblerHandler(packetG[0])
    if packetG[0] != -1:
        packet = packetG
        noTrama = 0
        if packet[0] == 1:
    ...
    # if robot.ball_position[0] > 11 and abs(robot.ball_position[1]) < 1:
    #     if ROBOTID == 1:
    #         robotNode.getField('translation').setSFVec3f([-10, 0, 0.07])
    #         x = random.SystemRandom().uniform(-10, 10)
    #         y = random.SystemRandom().uniform(-6, 6)
    ...
    #     robot.rootNode.setVelocity([0, 0, 0, 0, 0, 0])
    #     robot.ballNode.setVelocity([0, 0, 0, 0, 0, 0])
    #     robot.connector.unlock()
