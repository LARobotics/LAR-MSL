...
class Robot:
    ...
    def dribblerHandler(self, skill):
        ...
        #print(dist2ball, self.ball_handler)
        if(dist2ball < 0.2 and self.ball_handler == 0 and time.time()-self.lastKickTime > 0.5 and (skill == 2 or skill == 3 or skill == 4)):
            self.ball_handler = 1
            self.connector.lock()
    ...
    def KICK(self, args):                           # NOT READY!
        ...
        if self.ball_handler == 1:
            if abs(self.orientate(args[0], args[1])) < 0.1:
                ...
                if abs(self.position[0]-args[3]) > MARGIN or abs(self.position[1]-args[4]) > MARGIN:
                    dist, ang = cart2pol(args[3]-self.position[0], args[4]-self.position[1])
                    x, y = pol2cart(2, ang)        
                    self.rootNode.setVelocity([x, y, 0, 0, 0, angVel])
                else:
                    self.rootNode.setVelocity([0, 0, 0, 0, 0, angVel])
    ...
    def DEFEND(self, args):
        ...
        self.MOVE([XDEFEND, args[1], 1, args[0], args[1], 0, 0], 2, one=1)
        return 0
