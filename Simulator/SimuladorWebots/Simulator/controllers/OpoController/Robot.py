...
class Robot:
    def __init__(self, supervisor, RobotName):
        ...
        self.dispSocket.bind(("localhost", 32200+int(RobotName[-1])))
        ...
        self.localSocket.bind(("localhost", 32290+int(RobotName[-1])))
        #self.localSocket.settimeout(0.001)
        self.localSocket.setblocking(0)
        
        b = ["Opo1", "Opo2", "Opo3", "Opo4", "Opo5"]
        b.remove(RobotName)
    ...
    def getInfo(self):
        ...
        ball_position1[2] = 40 #round(self.ball_position[0]*FIELD_DIMENSIONS["A"]/220, 2)*100
        ...
        PC = [20, 30, 40, 50, 60, self.commID, "2f2fLG", self.commID, self.commID]
        self.commID += 1
        if self.commID >= 128:
            self.commID = 0
    ...
    def dribblerHandler(self, skill):
        if(self.robotID != 1):
            ...
            #print(dist2ball, self.ball_handler)
            if(dist2ball < 0.2 and self.ball_handler == 0 and time.time()-self.lastKickTime > 0.5 and (skill == 2 or skill == 3 or skill == 4 or skill == 7)):
                self.ball_handler = 1
                self.connector.lock()
    ...
    def MOVE(self, args, vel=3, one = 0):
        if one == 0:
    ...
    def send2Display(self, func, cao, args):
        ...
        self.dispSocket.sendto(packet.encode(), ("localhost", 32200))
