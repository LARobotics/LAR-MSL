...
def strategyTEMP(Robots, kick, recebe):
    ...
    for robot in Robots:
        for i in robot.otherRobots:
            OpponentRobotLocations.append([i[0:2]])# for robot in Robots[0:4]])#(np.array(Robots[0].otherRobots))[0:4]
    ...
    #     Robots[row[i]].nearestOpponent = Robots[row[i]].otherRobots[col[i]]
        # Robots[row[i]].probField[3].field = consts.zonesFields[col[i]]
    ...
    for i, s in enumerate(strat):
        if s == "KICK":
            ...
            if idRecieve == -1:
                idRecieve = -1
                Robots[BallHandler-1].packet = [KICK, consts.FIELD_DIMENSIONS["A"]/20, 0, 0, 0, 0, 0]
        ...
        # for robot in Robots:
        #     print(robot.robotID, robot.packet[0], end= ' ')
        #     print(strat, ss)
