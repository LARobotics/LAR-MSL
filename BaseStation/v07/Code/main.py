...
if REPRESENT and runMode:
    ...
    ############ MENU TABS ############
    MenuSelected = 0
    MainMenus = [["Game + RefBox", "Control", "Skills Test", "Calibration", "Exit"],[0, 0, 0, 0, 0],[],[Game, Control, Skills, Skills, Exit]]
...
def main():
    ...
    if finalTime > TIMEPERLOOP:
        print(f"{consts.bcolors.FAIL}{finalTime}ms{consts.bcolors.ENDC}", end=" | ")
    elif finalTime > TIMEPERLOOP*0.8:
        print(f"{consts.bcolors.WARNING}{finalTime}ms{consts.bcolors.ENDC}", end=" | ")
    else:
        print(f"{consts.bcolors.ENDC}{finalTime}ms{consts.bcolors.ENDC}", end=" | ")
