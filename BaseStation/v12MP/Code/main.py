...
def changeCalibValues(selectedRobot):
    ...
    if tempValues != Robots[selectedRobot].calibSkillValues[consts.calibSkillPIDCode]:
        print(selectedRobot, consts.calibSkillPIDCode, Robots[selectedRobot].calibSkillValues[consts.calibSkillPIDCode])
...
def main():
    ...
    gui = pool.apply_async(guiMain, args=(kick, consts.MainMenus))

    # p = Thread(target=guiMain, args=(kick,consts.MainMenus,))
    # p.start()
    if consts.PRINTTIME:
        print(round((time.time()-loopTime)*1000, 2), "ms", end=' ')
    # if refBox.available:
