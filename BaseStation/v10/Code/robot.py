...
class Robot:
    ...
    def draw_robotInfo(self, selectedRobot = 0):
        ...
        if self.battery < 11.9:
            pygame.draw.rect(consts.SCREEN,consts.COLORS["batRed"],(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + int(int(consts.RESOLUTION[0]/consts.NUMBER_OF_ROBOTS)/4), 167 * consts.FACTOR,int(consts.RESOLUTION[0] /consts.NUMBER_OF_ROBOTS) - 55*consts.FACTOR, 7 *consts.FACTOR,),border_radius=5,)
            emoji = "\U0001FAAB"
        elif self.battery < 12.2:
            pygame.draw.rect(consts.SCREEN,consts.COLORS["batYellow"],(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)  + int(int(consts.RESOLUTION[0]/consts.NUMBER_OF_ROBOTS)/4), 167 * consts.FACTOR,int(consts.RESOLUTION[0] /consts.NUMBER_OF_ROBOTS) - 55*consts.FACTOR, 7 *consts.FACTOR,),border_radius=5,)
            emoji = "\U0001F50B"
        else:
            pygame.draw.rect(consts.SCREEN,consts.COLORS["batGreen"],(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + int(int(consts.RESOLUTION[0]/consts.NUMBER_OF_ROBOTS)/4), 167 * consts.FACTOR,int(consts.RESOLUTION[0] /consts.NUMBER_OF_ROBOTS) - 55*consts.FACTOR, 7 *consts.FACTOR,),border_radius=5,)
            emoji = "\U0001F50B"
        text = consts.SMALLFONT.render(emoji+ str(self.battery) + " V", True, (255, 255, 255))
        consts.SCREEN.blit(text,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + int(int(consts.RESOLUTION[0]/consts.NUMBER_OF_ROBOTS)/4) + 2 * consts.FACTOR, 160 *consts.FACTOR + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR,),)

        if self.pcBattery < 11.9:
            pygame.draw.rect(consts.SCREEN,consts.COLORS["batRed"],(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + int(3*int(consts.RESOLUTION[0]/consts.NUMBER_OF_ROBOTS)/5), 167 * consts.FACTOR,int(consts.RESOLUTION[0] /consts.NUMBER_OF_ROBOTS) - 50*consts.FACTOR, 7 *consts.FACTOR,),border_radius=5,)
            emoji = "\U0001FAAB"
        elif self.pcBattery < 12.2:
            pygame.draw.rect(consts.SCREEN,consts.COLORS["batYellow"],(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + int(3*int(consts.RESOLUTION[0]/consts.NUMBER_OF_ROBOTS)/5), 167 * consts.FACTOR,int(consts.RESOLUTION[0] /consts.NUMBER_OF_ROBOTS) - 50*consts.FACTOR, 7 *consts.FACTOR,),border_radius=5,)
            emoji = "\U0001F50B"
        else:
            pygame.draw.rect(consts.SCREEN,consts.COLORS["batGreen"],(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + int(3*int(consts.RESOLUTION[0]/consts.NUMBER_OF_ROBOTS)/5), 167 * consts.FACTOR,int(consts.RESOLUTION[0] /consts.NUMBER_OF_ROBOTS) - 50*consts.FACTOR, 7 *consts.FACTOR,),border_radius=5,)
            emoji = "\U0001F50B"
        text = consts.SMALLFONT.render("\U0001F4BB" + emoji+ str(self.pcBattery) + " V", True, (255, 255, 255))
        consts.SCREEN.blit(text,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + int(6.25*int(consts.RESOLUTION[0]/consts.NUMBER_OF_ROBOTS)/10), 160 *consts.FACTOR + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR,),)

        text = consts.SMALLFONT.render("\U0001F30D " + str(round(self.position[0]/220*consts.FIELD_DIMENSIONS["A"], 2)) + " " + str(round(self.position[1]/140*consts.FIELD_DIMENSIONS["B"], 2)) + " " + str(self.position[2]), True, (255, 255, 255))
        ...
        text = consts.SMALLFONT.render("\U000026BD " + str(round(self.ball_position[0]/220*consts.FIELD_DIMENSIONS["A"], 2)) + " " + str(round(self.ball_position[1]/140*consts.FIELD_DIMENSIONS["B"], 2)) + " " + str(self.ball_position[2]), True, (255, 255, 255))
        consts.SCREEN.blit(text,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + 38 * consts.FACTOR,160 *consts.FACTOR + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 7 *consts.FACTOR,),)
        text = consts.SMALLFONT.render("\U0001F4E9 " + consts.SKILLSGUI[self.packet[0]] + " " + str(self.packet[1:]), True, (255, 255, 255))
        ...
        text = consts.SMALLFONT.render("QC1: " + str(self.QC1) + "%", True, (255, 255, 255))
        ...
        text = consts.SMALLFONT.render("QC2: " + str(self.QC2) + "%", True, (255, 255, 255))
        consts.SCREEN.blit(text,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 20 * consts.FACTOR+ 2 *consts.FACTOR,160 *consts.FACTOR + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 21 *consts.FACTOR,),)
...
def getInfo(Robots):
    ...
    for robot in Robots:
        try:
            ...
            robot.position[0] = round(robot.position[0]/(1000/(220/20)), 2)
            robot.position[1] = round(robot.position[1]/(1000/(140/20)), 2)
            robot.position.append(int(robot.info[1]))
            robot.orientation = int(robot.info[1])
            robot.ball_position = [float(num) for num in robot.info[2].split(",")]
            robot.ball_position.append(int(0))
            robot.ball_handler = int(robot.info[3])
            robot.ball_position[0] = round(robot.ball_position[0]/(1000/(220/20)), 2)
            robot.ball_position[1] = round(robot.ball_position[1]/(1000/(140/20)), 2)
            # print(robot.position, robot.orientation, ball_position, ball_handler)
            ...
            # print(robot.robotID, robot.info[4])
            if robot.info[4] != "":
                ...
                for r in range(len(robot.otherRobots)):
                    robot.otherRobots[r][0] = round(robot.otherRobots[r][0]/(1000/(220/20)), 2)
                    robot.otherRobots[r][1] = round(robot.otherRobots[r][1]/(1000/(140/20)), 2)
            else:
                # TODO NEED TO FIX THIS IN THE REAL WORLD
                pass
                # robot.otherRobots = [[-10, -10], [-10, -10], [-10, -10], [-10, -10], [-10, -10]]
            a = 1
            # robot.otherRobots=[]
        except:
            # robot.position = [-12.5, 8.5, 0]
            pass
    # print(Robots[3].otherRobots)
    return a
