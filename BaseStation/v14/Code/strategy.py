...
def mainStrategy(Robots, kick, recebe):
    ...
    if BallHandler > 0:
        ...
        #! TO CHECK IF THIS HELPS ON STABILITY
        #! IT DOENST WORK
        
        # if overallProbTemp >= prevOverallProb + HISTERESPROB:
        #     solution = solutionTemp
        #     solutionProbs = solutionProbsTemp
        #     overallProb = overallProbTemp
        #     overAllProbCounter = 0
        # elif overallProbTemp < prevOverallProb - HISTERESPROB:
        #     # overAllProbCounter += 1
        #     # if overAllProbCounter >= 30:
        #     solution = solutionTemp
        #     solutionProbs = solutionProbsTemp
        #     overallProb = overallProbTemp
        # elif len(solutionProbs) == 0:
        #     solution = solutionTemp
        #     solutionProbs = solutionProbsTemp
        #     overallProb = overallProbTemp
        # else:
        ...
        # print(solutionProbs, end=" | ")
        # print(overallProb, end= " | ")
        # print(overallProbTemp)
        ...
        if len(solution) > 1:
            ...
            for i in range(len(solution2)-2):
                guiElements.drawLine(Robots[solution2[i]-1].position, Robots[solution2[i+1]-1].position, color=COLORS["batYellow"], width=2, text=str(solutionProbs2[i]))
            guiElements.drawLine(Robots[solution2[-2]-1].position, (consts.FIELD_DIMENSIONS["A"]/20, 0), color=COLORS["batYellow"], width=2, text=str(solutionProbs2[-1]))
    ...
    if BallHandler != 0:
        ...
        if Shoot and len(solution)>1:
            if solution[1] == 0 and solutionProbs[0] >= MINPROBGOAL + HISTERESEGOAL:
                Goal = 1 
            elif solution[1] == 0 and solutionProbs[0] <= MINPROBGOAL - HISTERESEGOAL:#solutionProbs[0] <= MINPROBPASS - HISTERESEPASS:
                Goal = 0
                
        if solutionProbs[0] >= MINPROBPASS + HISTERESEPASS:
            Pass = 1 
        elif solutionProbs[0] <= MINPROBPASS - HISTERESEPASS:
            Pass = 0
    ...
    for i, s in enumerate(strat):
        match s:
            case "KICK":     #?#################### KICK #####################
                ...
                Robots[BallHandler-1].packet = [KICK, Robots[idRecieve].position[0], Robots[idRecieve].position[1], 0, Robots[BallHandler-1].moveTo[0], Robots[BallHandler-1].moveTo[1], 0]
                ...
                if idRecieve == -1:
                    Robots[BallHandler-1].packet = [KICK, consts.FIELD_DIMENSIONS["A"]/20, 0, 1, Robots[BallHandler-1].moveTo[0], Robots[BallHandler-1].moveTo[1], 0]
            ...
            case "MOVE":      #?#################### MOVE #####################
                for i, a in enumerate(ss):
                    if a == 0:
                        # print(i, "MOVE", end="")
                        Robots[i].packet = [MOVE, Robots[i].moveTo[0], Robots[i].moveTo[1], 0, 0, 0, 0]
    ...
    if opo == 0:
        ...
        if consts.NUMBERofJoySticks == 1:
            if 7 in joystick[0] and consts.TempJoyStickPointer < 4:
                consts.SimOpponents[consts.JoyStickPointer].packet = [CONTROL, 0, 0, 0, 0, 0, 0]
            ...
            for i, opo in enumerate(consts.SimOpponents):
                # if i in controlIDSList:
    ...
    match command:
        ...
        case "FREEKICK":    
            if message in refBox.myIPs:    #Nosso FREEKICK
                ...
                Robots[4].packet = [MOVE, -5, -FIELD_DIMENSIONS["C"]/10-FIELD_DIMENSIONS["L"]/20, 0, 0, 0, 0]
        case "KICKOFF":     
            if message in refBox.myIPs:    #Nosso KICKOFF
                Robots[0].packet = [MOVE, -9, -FIELD_DIMENSIONS["C"]/10-FIELD_DIMENSIONS["L"]/20, 0, 0, 0, 0]
                Robots[1].packet = [MOVE, -8, -FIELD_DIMENSIONS["C"]/10-FIELD_DIMENSIONS["L"]/20, 0, 0, 0, 0]
                Robots[2].packet = [MOVE, -7, -FIELD_DIMENSIONS["C"]/10-FIELD_DIMENSIONS["L"]/20, 0, 0, 0, 0]
                Robots[3].packet = [MOVE, -6, -FIELD_DIMENSIONS["C"]/10-FIELD_DIMENSIONS["L"]/20, 0, 0, 0, 0]
