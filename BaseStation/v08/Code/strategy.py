...
def strategyTEMP(Robots, kick, recebe):
    ...
    if BallHandler > 0:
        playersMap = decisionMaking.calculateGraph(Robots, BallHandler, show = 1)
        ...
        if len(solution) > 1:
            ...
            for i in range(len(solution2)-2):
                guiElements.drawLine(Robots[solution2[i]-1].position, Robots[solution2[i+1]-1].position, color=COLORS["batYellow"], width=3)
            guiElements.drawLine(Robots[solution2[-2]-1].position, (11, 0), color=COLORS["batYellow"], width=3)
...
def ControlRobots(keyboard, joystick, Robots, selectedRobot):
