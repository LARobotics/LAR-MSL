...
dists_calibration=[30,40,50,60,70,80,90,100,110,120,150,180,210,240,270,320,370,420,480,540,640,760,900]
...
def save_map_field(values):
    ...
    #HORIZONTAL LINES
    for i in range(int((MAP_MARGIN_SIZE_W/2)-round(B/2)),int((MAP_MARGIN_SIZE_W/2)+round(B/2)+1)):
        #MAP_FIELD[int(MAP_MARGIN_SIZE_H/2)-1][i]=120
        #MAP_FIELD[int(MAP_MARGIN_SIZE_H/2)+1][i]=120
        MAP_FIELD[int(MAP_MARGIN_SIZE_H/2)][i]=255
        MAP_FIELD[int(MAP_MARGIN_SIZE_H/2+round(A/2))][i]=255
        MAP_FIELD[int(MAP_MARGIN_SIZE_H/2-round(A/2))][i]=255
    #HORIZONTAL LINES AREA
    for i in range(int((MAP_MARGIN_SIZE_W/2)-round(C/2)),int((MAP_MARGIN_SIZE_W/2)+round(C/2)+1)):
        MAP_FIELD[int((MAP_MARGIN_SIZE_H/2)+(A/2)-E)][i]=255
        MAP_FIELD[int((MAP_MARGIN_SIZE_H/2)-(A/2)+E)][i]=255
    
    #HORIZONTAL LINES SMALL AREA
    for i in range(int((MAP_MARGIN_SIZE_W/2)-round(D/2)),int((MAP_MARGIN_SIZE_W/2)+round(D/2)+1)):
        MAP_FIELD[int((MAP_MARGIN_SIZE_H/2)+(A/2)-F)][i]=255
        MAP_FIELD[int((MAP_MARGIN_SIZE_H/2)-(A/2)+F)][i]=255
    #VERTICAL LINES
    for i in range(int((MAP_MARGIN_SIZE_H/2)-round(A/2)),int((MAP_MARGIN_SIZE_H/2)+round(A/2)+1)):
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)-round(B/2))]=255
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)+round(B/2))]=255
    #VERTICAL LINES AREA 1
    for i in range(int((MAP_MARGIN_SIZE_H/2)-round(A/2)),int((MAP_MARGIN_SIZE_H/2)-round(A/2)+E+1)):
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)-round(C/2))]=255
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)+round(C/2))]=255
    #VERTICAL LINES AREA 2
    for i in range(int((MAP_MARGIN_SIZE_H/2)+round(A/2)-E),int((MAP_MARGIN_SIZE_H/2)+round(A/2)+1)):
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)-round(C/2))]=255
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)+round(C/2))]=255
    #VERTICAL LINES SMALL AREA 1
    for i in range(int((MAP_MARGIN_SIZE_H/2)-round(A/2)),int((MAP_MARGIN_SIZE_H/2)-round(A/2)+F+1)):
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)-round(D/2))]=255
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)+round(D/2))]=255
    #VERTICAL LINES SMALL AREA 2
    for i in range(int((MAP_MARGIN_SIZE_H/2)+round(A/2)-F),int((MAP_MARGIN_SIZE_H/2)+round(A/2)+1)):
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)-round(D/2))]=255
        MAP_FIELD[i][int((MAP_MARGIN_SIZE_W/2)+round(D/2))]=255
    #CENTER CIRCLES
...
tabgrp = [
    [sg.Menu(menu_def,text_color='black', font="SYSTEM_DEFAULT", pad=(10,10))],
        ...
        [sg.TabGroup([[
            sg.Tab('Map Calibration', layout2,title_color='Blue', element_justification= 'center'),
            sg.Tab('Warp', layout3,title_color='Black',tooltip='Warp', element_justification= 'center'),
            sg.Tab('HSV Calibration', layout1, title_color=('Black'), border_width =100,tooltip='HSV Calibration', element_justification= 'center'),
            sg.Tab('Field Calibration', layout4, title_color=('Black'), border_width =100,tooltip='Field Calibration', element_justification= 'center')
            
            ]], tab_location='centertop',title_color='white',background_color='firebrick4', tab_background_color='black',selected_title_color='black',selected_background_color='white', border_width=5)],
        ]  
        ...
        if(count>6):
            ...
            for index,x in enumerate(x_line):
                ...
                if(0<=index<300 and 0<=x<300  ):
                    if 0>int(300-(y_line[index]*0.5)):
                        plot_func[299][x][2]=255
                    elif int(300-(y_line[index]*0.5))>=300:
                        plot_func[0][x][2]=255
                    else:
                        plot_func[int(299-(y_line[index]*0.5))][x][2]=255
