...
# server_address = ('172.16.49.157', 28097)
server_address = ('172.16.1.2', 28097)
...
Arg = ""
CommandID = -1
myIPs = ["172.16.49.0", "172.16.49.1", "127.0.0.1", "224.16.32.127", "224.16.32.49"]
state = 1
...
try:
    ...
    # refBoxSocket.bind(('172.16.49.17', 49738))

    print(server_address)
    refBoxSocket.settimeout(5)
    print(server_address)
    refBoxSocket.connect(server_address)
    print(server_address)
    refBoxSocket.setblocking(0)
    print(server_address)
    available = 1
...
def handleComms():
    global CommandID, Arg, startRefBoxTime
    # if available:
    try:
        if time.time()-startRefBoxTime > 1:
            startRefBoxTime = time.time()
        ...
        if str(e) != "timed out" and str(e) != "[WinError 10053] An established connection was aborted by the software in your host machine" and str(e) != "1 [WinError 10035] A non-blocking socket operation could not be completed immediately":
            pass#print(1,e)
...
def checkMessage(message):
    #print(message["command"], message["targetTeam"])
...
def handleCommand(command, message, Robots):
    global CommandID, Message, Arg, myIPs
    CommandID = command
    Message = message
    Arg = message
