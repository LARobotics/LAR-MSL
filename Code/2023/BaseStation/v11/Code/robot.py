...
class Robot:
    ...
    def draw_robot(self, selectedFlag = 0):
        ...
        pygame.draw.circle(consts.SCREEN,consts.COLORS["yellow"],(self.ball_position[0] * 10 * consts.FACTOR + consts.FIELD_SIZE["wall"][1] / 2,-self.ball_position[1] * 10 *consts.FACTOR+ consts.FIELD_SIZE["wall"][1] / 2+ consts.YOFFSET,),consts.FACTOR,)
    ...
    def draw_robotInfo(self, selectedRobot = 0):
        ...
        if self.battery < 0:
            ...
            if self.battery < 15:
                ...
                emoji = "\U0001F50B"
            text = consts.SMALLFONT.render(emoji+ str(self.battery) + "%", True, cor)#(255, 255, 255))
        ...
        if self.battery12 < 0:
            cor = consts.COLORS["pink"]
            emoji = "\U0001FAAB"
            text = consts.SMALLFONT.render(emoji+ str("NOT CON."), True, cor)#(255, 255, 255))
        else:
            if self.battery12 < 15:
                ...
                cor = consts.COLORS["white"]
                emoji = "\U0001F50B"
            text = consts.SMALLFONT.render(str("\U0001F4F7")+emoji+ str(self.battery12) + "%", True, cor)#(255, 255, 255))
        ...
        if self.pcbattery < 15:
            cor = consts.COLORS["batRed"]
            emoji = "\U0001FAAB"
        elif self.pcbattery < 30:
            cor = consts.COLORS["batYellow"]
            emoji = "\U0001F50B"
        else:
            cor = consts.COLORS["white"]
            emoji = "\U0001F50B"
        text = consts.SMALLFONT.render("\U0001F4BB"+emoji+ str(self.pcbattery) + "%", True, cor)#(255, 255, 255))
        ...
        text = consts.SMALLFONT.render("\U000026BD " + str(round(self.ball_position[0], 2)) + " " + str(round(self.ball_position[1], 2)) + " " + str(self.ball_position[2]), True, (255, 255, 255))
        ...
        text = consts.SMALLFONT.render("\U0001F4F6 " + str(self.QC1) + "%", True, (255, 255, 255))
        ...
        text = consts.SMALLFONT.render("OMNI: " + str(self.Omni) + "%", True, (255, 255, 255))
        consts.SCREEN.blit(text,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 20 * consts.FACTOR+ 2 *consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 18 *consts.FACTOR,),)
        "\U0001F321"
        text = consts.SMALLFONT.render("OMNI: " + str(self.Omni) + "%", True, (255, 255, 255))
        consts.SCREEN.blit(text,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 20 * consts.FACTOR+ 2 *consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 18 *consts.FACTOR,),)
...
def getInfo(Robots):
    ...
    for robot in Robots:
        try:
            ...
            # print(len(robot.otherRobots), end=' - ')
            # if(robot.robotID == 5):
            #     print()
            # print("7 - ESP", robot.info[3], len(robot.info[3]))
        ...
        #     print(robot.robotID, newMessage)
        #     logFile.log_message(str(str(robot.robotID) + ";" + str(newMessage)))
        #     robot.info = (newMessage.replace("]", "").replace("[", "").replace(" ", "").split(";"))
        #     robot.position = [float(num) for num in robot.info[0].split(",")]
        #     robot.position[0] = round(robot.position[0], 2)/100#round(robot.position[0]/(1000/(consts.FIELD_DIMENSIONS["A"]/20)), 2)
        #     robot.position[1] = round(robot.position[1], 2)/100#round(robot.position[1]/(1000/(consts.FIELD_DIMENSIONS["B"]/20)), 2)
        ...
        #     robot.ball_position[0] = round(robot.ball_position[0], 2)/100#round(robot.ball_position[0]/(1000/(consts.FIELD_DIMENSIONS["A"]/20)), 2)
        #     robot.ball_position[1] = round(robot.ball_position[1], 2)/100#round(robot.ball_position[1]/(1000/(consts.FIELD_DIMENSIONS["B"]/20)), 2)
        ...
        #             robot.otherRobots[r][0] = round(robot.otherRobots[r][0], 2)/100
        #             robot.otherRobots[r][1] = round(robot.otherRobots[r][1], 2)/100
        #     else:
