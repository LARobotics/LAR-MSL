...
x = 200#consts.FIELD_DIMENSIONS["A"]# 240
y = 140#consts.FIELD_DIMENSIONS["B"]# 160
# x = 240
# y = 160
...
def initFields(robotID):
    ...
    #     a[b]UI.slider.setValue(float(fileHandling.getPointer(consts.mapConfigJsonPaths[0])[b]))
...
def circleMap(size_X, size_Y, infill = 1, power = 1, radius = -1, args = []):
    ...
    if infill == 3 and len(args) > 1:
        if args[1] < 0:
            matrixFinal = np.clip(matrixFinal, a_min=None, a_max=0)
