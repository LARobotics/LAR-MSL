...
class Robot:
    ...
    def draw_robot(self, selectedFlag = 0):
        if self.fps != -1:
            ...
            pygame.draw.circle(consts.SCREEN,consts.COLORS["yellowStealth"],(ball_xr, ball_yr),consts.FACTOR,)
            guiElements.drawLine(self.position, self.ball_position_robot, color=consts.COLORS["yellowStealth2"])
    ...
    def draw_robotInfo(self, selectedRobot = 0):
        ...
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + 15 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 3 *consts.FACTOR,),"FPS: " + str(f"{self.fps:^5}"), self.get_text_color(self.fps))
        ...
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + 50 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 3 *consts.FACTOR,),"\U00002601 : " + str(f"{self.commit:^7}"), (255, 255, 255))
        ...
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + 2 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 13 *consts.FACTOR,),"\U0001F30D  " + str(f"{round(self.position[0], 2):^5}") + " " + str(f"{round(self.position[1], 2):^5}") + " " + str(f"{self.position[2]:^5}") + "    \U000026BD  " + str(f"{round(self.ball_position[0], 2):^5}") + " " + str(f"{round(self.ball_position[1], 2):^5}") + " " + str(self.ball_handler), (255, 255, 255))
        ...
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 2 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 26 *consts.FACTOR,),"\U0001F4F6  " + str(f"{self.commsQuality:^5}") + "% | " + str(f"{self.ID:^3}") + "/128", self.get_text_color(self.commsQuality))
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 45 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 26 *consts.FACTOR,),"\U000026A1 \U000026BD : " + str(self.Cap) + " %", self.get_text_color(self.Cap))
        ...
        consts.SMALLFONT.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 2 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 38 *consts.FACTOR,),"\U00002197  " + str(f"{self.disp_db_l:^3}") + "  v-> " + str(f"{self.vel_db_l:^3}") + "  \U0001F94E  " + str(f"{self.vel_db_l:^3}") + " <-v  " + str(f"{self.disp_db_r:^3}") + "  \U00002196", (255, 255, 255))
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 2 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 44 *consts.FACTOR,),"CPU:" + str(f"{self.cpu:^3}") + "%", self.get_text_color(self.cpu))
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 22 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 43.9 *consts.FACTOR,),"\U0001F321:" + str(f"{self.cpuTemp:^3}") + "ºC", self.get_text_color(self.cpuTemp))
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 39 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 44 *consts.FACTOR,),"GPU:" + str(f"{self.gpu:^3}") + "%", self.get_text_color(self.gpu))
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS)+ 59 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 43.9 *consts.FACTOR,),"\U0001F321:" + str(f"{self.gpuTemp:^3}") + "ºC", self.get_text_color(self.gpuTemp))
        
    def get_text_color(self, text):
        match text:
            case self.cpu:
                if self.cpu > 80:   return consts.COLORS["batRed"]
                elif self.cpu > 50:   return consts.COLORS["batYellow"]
                else:   return consts.COLORS["white"]
            case self.gpu:
                if self.gpu > 80:   return consts.COLORS["batRed"]
                elif self.gpu > 50:   return consts.COLORS["batYellow"]
                else:   return consts.COLORS["white"]
            case self.cpuTemp:
                if self.cpuTemp > 75:   return consts.COLORS["batRed"]
                elif self.cpuTemp > 50:   return consts.COLORS["batYellow"]
                else:   return consts.COLORS["white"]
            case self.gpuTemp:
                if self.gpuTemp > 75:   return consts.COLORS["batRed"]
                elif self.gpuTemp > 50:   return consts.COLORS["batYellow"]
                else:   return consts.COLORS["white"]
            case self.OMNI_temp:
                if self.OMNI_temp > 45:   return consts.COLORS["batRed"]
                elif self.OMNI_temp > 30:   return consts.COLORS["batYellow"]
                else:   return consts.COLORS["white"]
            case self.dt:
                if self.dt > 50:   return consts.COLORS["batRed"]
                elif self.dt > 34:   return consts.COLORS["batYellow"]
                else:   return consts.COLORS["white"]
            case self.Cap:
                if self.Cap > 75:   return consts.COLORS["white"]
                elif self.Cap > 50:   return consts.COLORS["batYellow"]
                else:   return consts.COLORS["batRed"]
            case self.commsQuality:
                if self.commsQuality > 90:   return consts.COLORS["white"]
                elif self.commsQuality > 70:   return consts.COLORS["batYellow"]
                else:   return consts.COLORS["batRed"]
            case self.fps:
                if self.fps >= 15:   return consts.COLORS["white"]
                elif self.fps > 0:   return consts.COLORS["batYellow"]
                else:   return consts.COLORS["batRed"]
    ...
    for robot in Robots:
        try:
            ...
            if len(robot.info[4]) > 0:
                ...
                robot.pcbattery = float(robot.pcInfo[4]) 
                robot.dt = float(robot.pcInfo[5])
                robot.commit = str(robot.pcInfo[6].replace("\"", "").replace("'", ""))
                robot.ID = float(robot.pcInfo[7])
                
            robot.received_packages += 1
                
            if robot.ID > 64 and robot.commsReady == 0:
                robot.commsReady = 1
            if robot.ID < 64 and robot.commsReady == 1:
                # print(robot.received_packages)
                robot.commsReady = 0
...
def dataFusion(Robots):
    ...
    ball_positions = [[robot.ball_position[0], robot.ball_position[1], robot.ball_position[6]] for robot in Robots]
    total_x = 0
    total_y = 0
    total_weight = 0
    number = 0
