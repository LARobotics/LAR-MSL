...
class Robot:
        ...
        if self.fps != -1:
            for i, opponent in enumerate(self.otherRobots):
                ...
                if consts.FIELDGREEN:
                    pygame.draw.circle(consts.SCREEN,consts.COLORS["green"], (x, y), consts.ROBOT_SIZE,)
                else:
                    pygame.draw.circle(consts.SCREEN,consts.COLORS["red"], (x, y), consts.ROBOT_SIZE,)
    ...
    def draw_robotInfo(self, selectedRobot = 0):
        ...
        consts.SMALLFONT2.render_to(consts.SCREEN,(robotID * int(consts.RESOLUTION[0] / consts.NUMBER_OF_ROBOTS) + 2 * consts.FACTOR,consts.FIELD_SIZE["wall"][1] + consts.MENUS_SIZE * consts.RESOLUTION[1] + 2 *consts.FACTOR + 13 *consts.FACTOR,),"\U0001F30D  " + str(f"{round(self.position[0], 2):^5}") + " " + str(f"{round(self.position[1], 2):^5}") + " " + str(f"{self.position[2]:^5}") + "    \U000026BD  " + str(f"{round(self.ball_position_robot[0], 2):^5}") + " " + str(f"{round(self.ball_position_robot[1], 2):^5}") + " " + str(self.ball_handler), (255, 255, 255))
...
def dataFusion(Robots):
    ...
    ball_positions = [[robot.ball_position[0], robot.ball_position[1], robot.ball_position[6]] for robot in Robots if robot.fps != -1]
    total_x = 0
    total_y = 0
    total_weight = 0
    number = 0
...
class SimOpponent:
    ...
    def sendInfoOpo(self):
        ...
        if time.time()-self.lastPacketTime >= 0.05:
