...
capture = 1 
dists_calibration=[15,20,30,40,50,60,70,80,110,120,150,180,210,240,270,320,370,420,480,540,640,750,900,1000]
dists_calibration = np.array(dists_calibration)+24
label_ball_calibration=[15,30,50,80,110,150,180,240,320,420,540,640,750,900] #TESTAR O NUMEROP DE PONTOS Devem ser muitos
...
def save_file():
    ...
    for x in range(0,240):
        ...
        for y in range(0,240):
            file.write("{"+str(int(real_dist_omni[x][y][0]))+", "+str(int(real_dist_omni[x][y][1]))+"}")
            file.write(", ")
...
def main():
    ...
    global out
    window["-2MAXH2-"].update(value = 255)
    window["-2MINH2-"].update(value = 0)
    window["-2MAXS2-"].update(value = 255)
    window["-2MINS2-"].update(value = 0)
    window["-2MAXV2-"].update(value = 255)
    window["-2MINV2-"].update(value = 0)
    ...
    while True:
        ...
        if values['_TAB_GROUP_'] == "-TAB_CENTER-":
            ...
            x_array.append(25)
            y_array.append(15)
            frame_dist=cv.inRange(frame, (values["-2MINH2-"], values["-2MINS2-"], values["-2MINV2-"]), (values["-2MAXH2-"], values["-2MAXS2-"], values["-2MAXV2-"])) #Tresh
            ...
            for y in range(240):
                if(frame_dist[240+y][int(values["-OFF2-"])]!=last and count<len(dists_calibration)):
                    ...
                    last_index=(dists_calibration[count])
            ...
            if count<len(dists_calibration):
